...
public class Exporter extends AbstractExporter {
	...
	public File exportBPEL(final List<BPELPlan> plans, final CSARID csarId) {
		...
		try {
			...
			for (final TServiceTemplate serviceTemplate : servTemps) {
				...
				org.oasis_open.docs.tosca.ns._2011._12.TBoundaryDefinitions.Interfaces ifaces = boundary
						.getInterfaces();
				...
				for (final BPELPlan plan : plans) {
					if (plan.getServiceTemplate().getQName().equals(buildQName(defs, serviceTemplate))) {
						...
						if (!alreadySpecified) {
							...
							final org.oasis_open.docs.tosca.ns._2011._12.TExportedOperation.Plan newPlanRefElement = this.toscaFactory
									.createTExportedOperationPlan();
	...
	private TPlan generateTPlanElement(final BPELPlan generatedPlan) {
		...
		plan.setId(this.generatePlanId(generatedPlan));
		plan.setPlanLanguage(BPELPlan.bpelNamespace);

		return plan;
	}

	private String generatePlanId(BPELPlan plan) {
		return plan.getBpelProcessElement().getAttribute("name") + "_"
				+ plan.getTOSCAInterfaceName() + "_" + plan.getTOSCAOperationName();
	}
	...
	private String generateRelativePlanPath(final BPELPlan buildPlan) {
		return "Plans/" + this.generatePlanId(buildPlan) + ".zip";
	}
