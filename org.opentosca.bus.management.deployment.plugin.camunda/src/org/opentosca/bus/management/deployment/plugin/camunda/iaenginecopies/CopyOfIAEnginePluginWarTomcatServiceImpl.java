package org.opentosca.bus.management.deployment.plugin.camunda.iaenginecopies;
...
import org.opentosca.bus.management.deployment.plugin.camunda.Activator;
...
public class CopyOfIAEnginePluginWarTomcatServiceImpl {
    ...
    public void deployImplementationArtifact(final CSARID csarID, final File warFile) {
        ...
        if (warFile != null && warFile.exists()) {
            deployPath = deploy(csarID, warFile);
     ...
     * @return if deploying was successful. If <tt>null</tt> is returned, deploying wasn't
     *         successful. Otherwise the endpoint will be returned.
     *
     */
    private String deploy(final CSARID csarID, final File warFile) {
        ...
        if (isRunning()) {
            ...
            try {
                ...
                // in URIs/URLs
                convertedQname = getConvertedcsarID(csarID);
    ...
    private String getEndpointSuffix(final Document properties) {
        ...
        if (properties != null) {
            ...
            for (int i = 0; i < list.getLength(); i++) {
                ...
                if (containsEndpointSuffix(propNode)) {
