 ...
 * (IACandidateWrapper,DACandidateWrapper,..). The class also represents if there are complete
 * provisioning possible with the available template implementations.
 * </p>
...
public class OperationChain {
     ...
     *         true. There may be no IACandidate available, because there is no need for DA's to
     *         provision. In this case true is also returned.
     */
    ...
    /**
     * <p>
     * Executes the first found ProvisioningCandidate to execute provisioning operations with the
     * appropriate plugins set in the candidate. The order of calling each operation provisioning is
     ...
     * @return returns false only when execution of a plugin inside the ProvisioningCandidate
     *         failed, else true. There may be no ProvisioningCandidate available, because there is
