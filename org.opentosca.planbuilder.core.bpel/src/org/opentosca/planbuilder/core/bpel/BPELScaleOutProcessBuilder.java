...
import org.opentosca.planbuilder.model.plan.ScalingPlanDefinition.AnnotatedAbstractNodeTemplate;
import org.opentosca.planbuilder.model.plan.ActivityType;
import org.opentosca.planbuilder.model.plan.ScalingPlanDefinition;
...
public class BPELScaleOutProcessBuilder extends AbstractScaleOutPlanBuilder {
	...
	private void addRecursiveInstanceSelection(final BPELPlan plan, final PropertyMap map,
		...
		final String nodeInstanceVarName = this.instanceInitializer.findInstanceIdVarName(plan, nodeTemplate.getId(),
				true);
		...
		final String relationInstanceVarName = this.instanceInitializer.findInstanceIdVarName(plan,
				relationshipTemplate.getId(), false);

		// create response variable
		final QName anyTypeDeclId = nodeContext
				.importQName(new QName("http://www.w3.org/2001/XMLSchema", "any", "xsd"));
		...
		final String serviceTemplateUrlVarName = ServiceInstanceVariablesHandler
				.getServiceTemplateURLVariableName(nodeContext.getMainVariableNames());
		...
		try {
			...
			Node fetchNodeInstanceData = new BPELProcessFragments().createRESTExtensionGETForNodeInstanceDataAsNode(
					serviceTemplateUrlVarName, responseVarName, nodeTemplate.getId(),
					"source=$bpelvar[$" + relationInstanceVarName + "]");
		...
		final String xpathQuery = "//*[local-name()='NodeTemplateInstanceResources']/*[local-name()='NodeTemplateInstances']/*[local-name()='NodeTemplateInstance']/*[1]/*[local-name()='Links']/*[local-name()='Link']/@*[local-name()='href']/string()";
		try {

			Node queryNodeInstanceUrl = new BPELProcessFragments().createAssignVarToVarWithXpathQueryAsNode(
					"recursiveSelection_fetchNodeInstance" + System.currentTimeMillis(), responseVarName,
					nodeInstanceVarName, xpathQuery);
	...
	private void addRecursiveInstanceSelection(final BPELPlan plan, final PropertyMap map,
		...
		final BPELPlanContext relationContext = this.createContext(relationshipTemplate, plan, map);
		final String relationshipTemplateInstanceVarName = this.instanceInitializer.findInstanceIdVarName(plan,
				relationshipTemplate.getId(), false);
		...
		final String nodeTemplateInstanceVarName = this.instanceInitializer.findInstanceIdVarName(plan,
		...
		final QName stringTypeDeclId = relationContext
		...
		try {
			Node requestRelationInstance = new BPELProcessFragments()
					.createBPEL4RESTLightRelationInstancesTargetNodeInstanceQueryGETAsNode(serviceTemplateUrlVarName,
							relationshipTemplate.getId(), responseVarName, nodeTemplateInstanceVarName);
		...
		final String xpathQuery = "//*[local-name()='RelationshipTemplateInstanceResources']/*[local-name()='RelationshipTemplateInstances']/*[local-name()='RelationshipTemplateInstance']/[0]/*[local-name()='Links']/*[local-name()='Link']/@*[local-name()='href']/string()";
	...
	public AbstractPlan buildPlan(String csarName, AbstractDefinitions definitions, QName serviceTemplateId,
		...
		throw new RuntimeException(
				"Group-based generation is currently not implemented as a Scaling Plan Definition is inherently a group, and the models have to be refactors for this to be an easy job");
	}
	...
	public List<BPELPlan> buildScalingPlans(final String csarName, final AbstractDefinitions definitions,
		...
		final List<ScalingPlanDefinition> scalingPlanDefinitions = fetchScalingPlansDefinitions(
				serviceTemplate.getTopologyTemplate(), tags);

		for (final ScalingPlanDefinition scalingPlanDefinition : scalingPlanDefinitions) {

			final String processName = ModelUtils
					.makeValidNCName(serviceTemplate.getId() + "_scalingPlan_" + scalingPlanDefinition.name);
	...
	private List<ScalingPlanDefinition> fetchScalingPlansDefinitions(final AbstractTopologyTemplate topology,
		...
		for (final String scalingPlanName : scalingPlanNamesRaw) {
			...
			if (!scalingPlanName.trim().isEmpty() && tags.containsKey(scalingPlanName.trim())) {
				...
				final List<AbstractNodeTemplate> nodeTemplates = fetchNodeTemplates(topology,
				...
				final List<AbstractRelationshipTemplate> relationshipTemplates = fetchRelationshipTemplates(topology,
						scalingPlanNodesNEdgesRawValueSplit[1]);
				...
				final List<AnnotatedAbstractNodeTemplate> selectionStrategy2BorderNodes = fetchSelectionStrategy2BorderNodes(
	...
	private List<AnnotatedAbstractNodeTemplate> fetchSelectionStrategy2BorderNodes(
		...
		final Map<String, String> selectionStrategyBorderNodesMap = transformSelectionStrategyListToMap(
		...
		for (final String selectionStrategy : selectionStrategyBorderNodesMap.keySet()) {
			final AbstractNodeTemplate node = fetchNodeTemplate(topologyTemplate,
	...
	private AbstractRelationshipTemplate getFirstOutgoingInfrastructureRelation(
			final AbstractNodeTemplate nodeTemplate) {
	...
	@Override
	public List<QName> getSupportedGroupTypes() {
