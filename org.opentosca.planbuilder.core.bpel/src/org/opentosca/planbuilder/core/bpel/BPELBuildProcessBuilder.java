...
import org.opentosca.planbuilder.core.bpel.situations.SituationGuardRegistration;
import org.opentosca.planbuilder.core.bpel.situations.SituationTriggerRegistration;
...
public class BPELBuildProcessBuilder extends AbstractBuildPlanBuilder {
	...
	private SituationTriggerRegistration sitTriggerRegistration;
	private SituationGuardRegistration sitGuardRegistration;
	...
	private QName buildGroupType = new QName("http://opentosca.org/grouptypes/build", "BuildGroup");
	...
	public BPELBuildProcessBuilder() {
		try {
			...
			this.sitTriggerRegistration = new SituationTriggerRegistration();
			this.sitGuardRegistration = new SituationGuardRegistration();
	...
	public BPELPlan buildPlan(String csarName, AbstractDefinitions definitions, QName serviceTemplateId,
		...
		if (serviceTemplate != null) {
			...
			final AbstractPlan buildPlan = this.generatePOG(new QName(processNamespace, processName).toString(),
					definitions, serviceTemplate, nodeTemplates, relationshipTemplate);

			LOG.debug("Generated the following abstract prov plan: ");
			LOG.debug(buildPlan.toString());

			final BPELPlan newBuildPlan = this.planHandler.createEmptyBPELPlan(processNamespace, processName, buildPlan,
					"initiate");
			...
			this.sitTriggerRegistration.handle(serviceTemplate, newBuildPlan);
			this.sitGuardRegistration.handle(serviceTemplate, newBuildPlan);
	...
	public BPELPlan buildPlan(final String csarName, final AbstractDefinitions definitions,
		...
		AbstractServiceTemplate servTemp = this.getServiceTemplate(definitions, serviceTemplateId);
		if (servTemp != null) {
			return this.buildPlan(csarName, definitions, serviceTemplateId,
					servTemp.getTopologyTemplate().getNodeTemplates(),
					servTemp.getTopologyTemplate().getRelationshipTemplates());
		} else {
			return null;
		}
	}
	...
	public List<AbstractPlan> buildPlans(final String csarName, final AbstractDefinitions definitions) {
		...
		for (final AbstractServiceTemplate serviceTemplate : definitions.getServiceTemplates()) {
			...
			if (!serviceTemplate.hasBuildPlan()) {
				...
				// check if this serviceTemplate has groups defined

				for (AbstractGroup group : serviceTemplate.getGroups()) {
					if (this.supportedGroup(group.getType())) {
						Subgraph sg = this.getSubgraphBasedOnNodes(serviceTemplate.getTopologyTemplate(),
								group.getNodeTemplates());
						BPELPlan groupBuildPlan = this.buildPlan(csarName, definitions, serviceTemplateId, sg.getNodes(), sg.getRelations());
						
						String opName = this.getOperationName(group);
						String ifaceName = this.getInterfaceName(group);
						
						if(opName != null) {
							groupBuildPlan.setTOSCAOperationname(opName);
						}
						
						if(ifaceName != null) {
							groupBuildPlan.setTOSCAInterfaceName(ifaceName);
						}
						
						plans.add(groupBuildPlan);
					}
				}
	...
	private String getOperationName(AbstractGroup group) {
		if(ModelUtils.getPropertyNames(group.getProperties()).contains("OperationName")) {
			return ModelUtils.getPropertyValue(group.getProperties(), "OperationName");			
		} else {			
			return null;
		}
	}
	
	private String getInterfaceName(AbstractGroup group) {
		if(ModelUtils.getPropertyNames(group.getProperties()).contains("InterfaceName")) {
			return ModelUtils.getPropertyValue(group.getProperties(), "InterfaceName");			
		} else {			
			return null;
		}
	}
	...
	/**
	 * Checks whether the given QName is a supported groupType
	 * 
	 * @param qname a QName
	 * @return true if the QName denotes a supported group type
	 */
	private boolean supportedGroup(QName qname) {
		if (qname.equals(this.buildGroupType) || (qname.getNamespaceURI().equals(this.buildGroupType.getNamespaceURI())
				&& qname.getLocalPart().startsWith(this.buildGroupType.getLocalPart()))) {
			return true;
		} else {
			return false;
		}
	...
	public List<QName> getSupportedGroupTypes() {
		List<QName> qnames = Collections.emptyList();
		qnames.add(buildGroupType);
		return qnames;
	}
