...
public class BPELTerminationProcessBuilder extends AbstractTerminationPlanBuilder {
	...
	public BPELPlan buildPlan(String csarName, AbstractDefinitions definitions, QName serviceTemplateId,
		...
		if (serviceTemplate != null) {
			...
			final AbstractPlan newAbstractTerminationPlan = generateTOG(
					new QName(processNamespace, processName).toString(), definitions, serviceTemplate, nodeTemplates,
	...
	private List<BPELScopeActivity> runPlugins(final BPELPlan plan, final PropertyMap propMap) {
		...
		/*
		 * TODO/FIXME until we decided whether we allow type plugins that achieve
		 * termination, we just terminate each VM and Docker Container we can find
		 */
		for (final BPELScopeActivity templatePlan : plan.getTemplateBuildPlans()) {
			...
			if (templatePlan.getNodeTemplate() != null) {
				...
				if (org.opentosca.container.core.tosca.convention.Utils
						.isSupportedVMNodeType(templatePlan.getNodeTemplate().getType().getId())) {
					...
					for (final AbstractNodeTemplate infraNode : infraNodes) {
						if (org.opentosca.container.core.tosca.convention.Utils
								.isSupportedCloudProviderNodeType(infraNode.getType().getId())) {
