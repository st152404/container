...
public class BPELDefrostProcessBuilder extends AbstractDefrostPlanBuilder {
    ...
    public List<AbstractPlan> buildPlansForCSAR(final String csarName, final AbstractDefinitions definitions) {
        ...
        for (final AbstractServiceTemplate serviceTemplate : definitions.getServiceTemplates()) {
            ...
            final List<AbstractPlan> newDefreezePlans =
                buildPlansForServiceTemplate(csarName, definitions, serviceTemplateId);

            if (newDefreezePlans != null) {
                for (final AbstractPlan plan : newDefreezePlans) {
                    LOG.debug("Created Defreeze Plan " + plan.getId());
                }
                plans.addAll(newDefreezePlans);
            }
