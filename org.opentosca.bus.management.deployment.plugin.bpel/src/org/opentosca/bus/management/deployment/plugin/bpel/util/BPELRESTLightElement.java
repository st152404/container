package org.opentosca.bus.management.deployment.plugin.bpel.util;
...
/**
 * This class provides a mapping between Java and BPEL4RESTLight XML elements. It provides
 * funtionality conform to the rules in BPELRESTLightUpdater when updating URIs on the DOM Nodes.
 */
public class BPELRESTLightElement {
     ...
     * openTOSCA Core for the referenced HTTP service.</li>
     ...
     * @throws NoBPELRESTLightElementException when the given node isn't a extension activity from
     *         BPEL4RESTLight ODE Extension conforming the above rules
     */
    protected BPELRESTLightElement(final Node node) throws NoBPELRESTLightElementException {
        ...
        switch (BPELRESTLightElementType.valueOf(node.getLocalName().trim())) {
            ...
            default:
                ...
                throw new NoBPELRESTLightElementException("Node isn't a BPEL4RESTLight Element: Node local name  is "
                    + node.getLocalName() + " instead of PUT/POST/DELETE/GET");
        }
        ...
        if (this.uriNode == null) {
            ...
            throw new NoBPELRESTLightElementException(
                "Node isn't a BPEL4RESTLight Element: No uri attribute in DOM node");
        }
        String temp;
        try {
     ...
     *
     * @return a URI. The uri can be just a part of the real URI value in the DOM if it contains
     *         $bpelvar[bpelVariable]
     */
    public URI getURI() {
        switch (this.uriType) {
            case BPELVAR:
     ...
     */
    protected boolean setURI(final URI uri) throws URISyntaxException {
        String oldURI;
        String newAuth;
    ...
    private String getURIPart() {
        final String temp = this.uriValue;
     ...
     */
    private void setURIPart(final String newURIPart) {
        final String temp = this.uriValue;
        final String bpelPart = temp.substring(temp.indexOf("bpelvar["));
     ...
     * @throws NoBPELRESTLightElementException whenever the uri value doesn't apply to rules specified
     *         for BPEL4RESTLight elements
     */
    ...
    public boolean equals(final Object obj) {
        if (!(obj instanceof BPELRESTLightElement)) {
            ...
            final BPELRESTLightElement el = (BPELRESTLightElement) obj;
